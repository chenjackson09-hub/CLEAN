...
export function DashboardKpiCard({ href, value, label, sub, highlight = false }: Props) {
  const className = `block rounded-2xl p-4 shadow-2xl transition-shadow bg-white border ${
  ...
  // Cards that map to a screen link there; purely-informational ones (e.g. the
  // overall rating) render as a plain card.
  return href ? (

...
- **Customers can cancel their own bookings** from `/bookings`: tapping a `BookingCard` opens `BookingDetailModal` (the customer-side mirror of `CleanDetailModal` — same portal pattern, but uses the customer i18n system), which has a Cancel button (two-step confirm) wired to the `cancelBooking` server action. Only `pending`/`accepted` bookings are cancellable; the action sets `status = 'cancelled'`. It does **not** restore the carved-out time to the cleaner's availability or notify the cleaner — both are open follow-ups.
- **`/bookings` groups into five sections purely by `status` (see `page.tsx`):** confirmed (`accepted`), pending (`pending`), **"Refused requests" (`declined`** — the cleaner said no, or the request expired with no response), **"Cancelled" (`cancelled`** — the request was called off: the customer pressed Cancel, or a sibling was auto-cancelled when another cleaner got booked), and past cleans (`completed` only). Note the customer-side display maps an expired-but-still-`pending` row to `declined` (mirroring what `lib/expireRequests.ts` will write on the next cleaner page load), so expired requests land under "Refused requests". `declined` vs. `cancelled` is the refused-vs-called-off distinction; there is no `cancelled_by` column (an earlier draft added one — migration `0003` — but it was dropped once the split became status-only).
...
- **The cleaner can cancel an accepted clean** from inside `CleanDetailModal` (two-step confirm), wired to the `cancelClean` server action — shown only when `booking.status === 'accepted'`. It sets `status = 'cancelled'` (so the customer sees it under their `/bookings` "Cancelled" section) and `router.refresh()`es the dashboard. Like the customer-side `cancelBooking`, it does **not** restore the carved-out availability or notify the customer — both open follow-ups.
